...
# ??   --- not conditional operator (condition ? then : else)
    elsif (($slash eq 'm//') and /\G \? ((?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|\\[?:]|[^?:])*) \? ([cgimosxp]*) /oxmsgc) {
        return e_qr('', '?','?',$1,$2);
    }
...
sub classic_character_class {
    ...
    return {
        ...
        '\V' => '(?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^\x0A\x0B\x0C\x0D])',
...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        elsif ($char[$i] =~ m/\A \\x (?:[89][0-9A-Fa-f]|[Aa]0|[Ee][0-9A-Fa-f]|[Ff][0-9A-Ea-e]) \z/oxms) {
            if ($i < $#char) {
                $char[$i] .= $char[$i+1];
                splice @char, $i+1, 1;
            }
    ...
    $modifier =~ tr/i//d;
    if ($left_e > $right_e) {
        $re = join '', $ope, $delimiter, "$your_gap(?:", @char, '>]}' x ($left_e - $right_e), ')(?{$Ehp15::last_s_matched=0})', $end_delimiter, $modifier;
    }
    else {
        $re = join '', $ope, $delimiter, "$your_gap(?:", @char,                               ')(?{$Ehp15::last_s_matched=0})', $end_delimiter, $modifier;
    }
    return $re;
}
...
sub e_qr_q {
    ...
    $modifier =~ tr/i//d;
    return join '', $ope, $delimiter, "$your_gap(?:", @char, ')(?{$Ehp15::last_s_matched=0})', $end_delimiter, $modifier;
}
...
sub e_s1 {
    ...
    $modifier =~ tr/i//d;
    if ($left_e > $right_e) {
        $re = join '', $ope, $delimiter, "($your_gap)(?:", @char, '>]}' x ($left_e - $right_e), ')(?{$Ehp15::last_s_matched=1})', $end_delimiter, $modifier;
    }
    else {
        $re = join '', $ope, $delimiter, "($your_gap)(?:", @char,                               ')(?{$Ehp15::last_s_matched=1})', $end_delimiter, $modifier;
    }
    return $re;
}
...
sub e_s1_q {
    ...
    $modifier =~ tr/i//d;
    return join '', $ope, $delimiter, "($your_gap)(?:", @char, ')(?{$Ehp15::last_s_matched=1})', $end_delimiter, $modifier;
}
...
In present Japan, HP-15 is widely used on mainframe I/O, the personal computer,
